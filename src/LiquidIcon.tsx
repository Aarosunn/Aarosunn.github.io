...
function maskTexture(path: string, box: number, size: number, soften: number, fit: number) {
  const c = document.createElement('canvas')
  c.width = c.height = size
  const g = c.getContext('2d')!
  g.fillStyle = 'rgb(128, 0, 255)' // outside: R .5 (an id all the same), G 0, B 1
  g.fillRect(0, 0, size, size)
  const pad = size * (1 - 0.8 * fit) / 2 // room around the mark for the pillow's rim and the ascii trail
  g.save()
  g.translate(pad, pad)
...
/** the look on the site: the lab's v1 pillow (Aaron's pick, 2026-09-13) */
export const SITE_ICON: IconLook = { name: 'v1 pillow', field: 'pillow', ascii: false, scale: 0.9, speed: LIQUID.speed, hoverSpeed: LIQUID.speed * 3, soften: 0, note: "the cube's chrome pillow over the mark" }

function Liquid({ path, box, fit, look, hover, paused }: { path: string; box: number; fit: number; look: IconLook; hover: boolean; paused: boolean }) {
  const { gl, size } = useThree()
  const SIZE = 512
  const mask = useMemo(() => maskTexture(path, box, SIZE, look.soften, fit), [path, box, look.soften, fit])
  ...
  const Q = useMemo(() => {
    ...
    return {
      ...
      final: (() => { const u = finalUniforms(); u.u_colorBack.value = [0, 0, 0, 0]; return raw(finalFragment(), u, FINAL_VERT) })(), // outside the mark: transparent
    }
  }, [])
  ...
  useFrame((_, dt) => {
    if (paused && solved.current) return // hidden with the page around the cube: nothing to draw
    if (!solved.current) {
...
export function LiquidIcon({ path, box, fit = 1, href, label, look, px = 96, paused = false }: { path: string; box: number; fit?: number; href: string; label: string; look: IconLook; px?: number; paused?: boolean }) {
  ...
  return (
    <a className="liquid-icon" href={href} target="_blank" rel="noreferrer" aria-label={label} style={{ width: px, height: px }} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
      <Canvas dpr={[1, 2]} gl={{ antialias: false, alpha: true, powerPreference: 'low-power' }} frameloop="always" style={{ position: 'absolute', inset: 0 }}>
        <Liquid path={path} box={box} fit={fit} look={look} hover={hover} paused={paused} />
      </Canvas>
    </a>
  )
}
...
/** fit: the square mark drawn a little smaller so it weighs the same as the round one */
export const LINKEDIN = { box: 24, fit: 0.84, path: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' }
