/**
 * v3 surfaces: the paper.design trio (github.com/paper-design/shaders, Apache 2.0) rebuilt on
...
export type V3Material = THREE.MeshPhysicalMaterial & { u: Record<string, THREE.IUniform> }
...
export function makeV3(variant: Variant, p: Palette, half: number, shared: Shared): V3Material {
  const m = new THREE.MeshPhysicalMaterial(BASE[variant]) as V3Material
  m.u = {
    ...shared,
    uHalf: { value: half },
  ...
  if (inj) {
    ...
    m.customProgramCacheKey = () => `v3-${variant}`
  }
  return m
}
