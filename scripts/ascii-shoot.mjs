// ascii outline review: the lab cube per wind preset, then the site. Usage: V=v16 node scripts/ascii-shoot.mjs -> shots/ascii/
import { chromium } from 'playwright'
import { mkdirSync } from 'node:fs'
mkdirSync('shots/ascii', { recursive: true })
const V = process.env.V || 'v16'
...
await page.evaluate((V) => { window.__aar.setShaderVersion(V); window.__aar.setShaderSpin(false) }, V)
...
for (const w of ['version', 'tight', 'breeze', 'gale', 'storm']) {
  ...
  await page.screenshot({ path: `shots/ascii/${V}-${w}.png`, clip })
}
const pr = page.evaluate(() => window.__aar.paperTurn('y', 1, 1))
await page.waitForTimeout(260)
await page.screenshot({ path: `shots/ascii/${V}-storm-mid.png`, clip })
await pr
